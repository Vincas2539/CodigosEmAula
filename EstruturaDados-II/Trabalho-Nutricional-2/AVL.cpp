...
NodeAVL *AVL::Predecessor(string id) const {
...
NodeAVL *AVL::Successor(string id) const {
  NodeAVL *node = SearchInternal(m_Root, id);
...
NodeAVL *AVL::Search(string id) const { return SearchInternal(m_Root, id); }

NodeAVL *AVL::SearchInternal(NodeAVL *node, string &id) const {
  if (node == nullptr)
    return nullptr;
...
NodeAVL *AVL::Insert(string id, Alimento *alimento) {
  if (IsEmpty()) {
    m_Root = new NodeAVL(id, alimento);
    // m_Root->UpdateBalanceFactor();
    return m_Root;
  }

  return InsertInternal(m_Root, nullptr, id, alimento);
}

NodeAVL *AVL::InsertInternal(NodeAVL *node, NodeAVL *parent, string id,
                             Alimento *alimento) {
  if (node == nullptr)
    node = new NodeAVL(id, alimento, parent);
  else if (node->GetID() == id)
    return nullptr; // Error! Cannot insert duplicate.
  else if (node->GetID() > id)
    node->SetLeft(InsertInternal(node->GetLeft(), node, id, alimento));
  else if (node->GetID() < id)
    node->SetRight(InsertInternal(node->GetRight(), node, id, alimento));

  node = Balance(node);
  return node;
}

void AVL::Remove(string id) { RemoveInternal(m_Root, id); }

NodeAVL *AVL::RemoveInternal(NodeAVL *node, string id) {
  if (node == nullptr)
    return nullptr;
  else if (node->GetID() == id)
...
void AVL::Save(ofstream &arquivo) { SaveInternal(m_Root, arquivo); }

void AVL::SaveInternal(NodeAVL *no, ofstream &arquivo) {
  if (no != NULL) {
    SaveInternal(no->GetLeft(), arquivo);
    SaveInternal(no->GetRight(), arquivo);
    if (no != m_Root)
      arquivo << no->ToFile() << endl;
    else
      arquivo << no->ToFile();
  }
}

void AVL::imprimeEscolha(NodeAVL *no, char tipo, vector<string> *lista) {
  if (no != NULL) {
    imprimeEscolha(no->GetLeft(), tipo, lista);
    if (tipo == no->GetData()->getTipo()) {
      lista->push_back(no->GetData()->getNome());
    }
    imprimeEscolha(no->GetRight(), tipo, lista);
  }
}

void AVL::imprimeTudo(NodeAVL *no, vector<string> *lista) {
  if (no != NULL) {
    imprimeTudo(no->GetLeft(), lista);
    lista->push_back(no->GetData()->getNome());
    imprimeTudo(no->GetRight(), lista);
  }
}

string AVL::imprimePorcao(NodeAVL *no, string nome) {
  string porcao;
  while (true) {
    if (no->GetData()->getNome() == nome){
      porcao = no->GetData()->getPorcao();
      if (porcao == "0")
        return "";
      return porcao;
    }
    else if (nome < no->GetData()->getNome()) {
      if (no->GetLeft() == NULL)
        return "";
      no = no->GetLeft();
    } else if (nome > no->GetData()->getNome()) {
